...
def parse_md5mesh(input_file):
    ...
    Vert = namedtuple("Vert", "st startWeight countWeight normal")
        ...
        for line in md5mesh_file:
            ...
            if mode == "root":
                ...
                if cmd == '}':
                    ...
                    # was 7, added 5 for normal
                    # ( x y z )
                    assert_num_args('vert', nargs, 12, tokens)
                    if numverts is None:
                    ...
                    if tokens[7] != '(':
                        raise MD5FormatError(f"Unexpected token 8 for vert': {tokens}")
                    if tokens[11] != ')':
                        raise MD5FormatError(f"Unexpected token 11 for vert': {tokens}")

                    startWeight = int(tokens[5])
                    countWeight = int(tokens[6])
                    normal = Vector(float(tokens[8]), float(tokens[9]), float(tokens[10]))
                    ...
                    verts[index] = Vert(st, startWeight, countWeight, normal)
